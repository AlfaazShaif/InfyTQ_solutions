...
class Train:
    ...
    def check_vacancy(self):
        count = 0
        temp = self.__compartment_list.get_head()
        # seat_vacant_percent = 0
        while temp:
            seat_vacant_percent = (temp.get_data().get_total_seats() - temp.get_data().get_no_of_passengers()) / temp.get_data().get_total_seats()
            if seat_vacant_percent > 0.5:
                count += 1
            temp = temp.get_next()
        return count  
